          ...
          return (
            <button
              ...
              className={`px-3 py-1 rounded-full text-sm font-medium transition-all duration-200 cursor-pointer ${
                isSelected ? 'bg-[#36C986] text-white shadow-md' : 'bg-white text-gray-700 border border-gray-200 hover:border-[#36C986]/50 hover:bg-[#36C986]/10'
              }`}
              onClick={() => setSelectedGradientColorKey(colorKey)}>
              {lang === 'en' ? colorData.en ?? colorData.name : colorData.name}
            </button>
          )
        })}
      </div>
          ...
          {currentGradients.map((gradient, index) => {
            ...
            return (
              <div
                ...
                {/* 选中状态 */}
                {isSelected && (
                  <div className='absolute right-4 top-4 w-6 h-6 bg-[#36C986] flex items-center justify-center rounded-full'>

  ...
  return (
    <div className='flex items-center'>
      <img
        ...
        loading='lazy'
        src={icon.label === '本地图标' || icon.label === 'Local icon' ? icon.value : `${iconifyHost}/${icon.value}.svg`}
        alt={`${icon.label} icon`}
      />
...
const IconSelect = () => {
  ...
  useEffect(() => {
    if (coverSetting.customIcon) {
      setSelectItem({ value: coverSetting.customIcon, label: t('icon.local') })
      setOptions([{ value: coverSetting.customIcon, label: t('icon.local') }])
      setLoading(false)
      setQuery('')
      return
    }
  ...
  const handleCustomIconChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setSelectItem({ value: customIcon, label: t('icon.local') })
      setOptions([{ value: customIcon, label: t('icon.local') }])
  ...
  if (loading) return <div className='flex-1 h-9 px-3 py-2 border border-input rounded-md text-sm'>{t('icon.loading')}</div>

  return (
    <div className='flex-1 flex items-center gap-2 min-w-0'>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button variant='outline' role='combobox' aria-expanded={open} className='flex-1 justify-between h-9 px-2'>
            <FormatOptionLabel icon={selectItem} />
            <ChevronDown className='opacity-50 shrink-0 ml-1' />
          </Button>
        </PopoverTrigger>
        <PopoverContent className='p-0'>
          <Command>
            <CommandInput value={query} placeholder={t('icon.searchPlaceholder')} className='h-9' onInput={handleInputChange} />
            <CommandList>
              <CommandEmpty>{t('icon.noResult')}</CommandEmpty>
              <CommandGroup>
                {options.map((item) => (
      ...
      <div className='relative h-9 w-16 shrink-0'>
        ...
        />
        <Button className='cursor-pointer h-9 w-full px-2'>{t('icon.upload')}</Button>
      </div>
    </div>
  )
}

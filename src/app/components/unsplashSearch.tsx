  ...
  return (
    <div className='ignore w-full flex-1 flex flex-col overflow-hidden'>
        ...
        <div className='max-w-2xl mx-auto'>
          <div className='relative pr-2 flex justify-between items-center bg-white rounded-lg border border-gray-200 hover:shadow-sm transition-shadow duration-200'>
            <input
              type='text'
              value={text}
              placeholder={t('uns.searchPlaceholder')}
      ...
      <div className='flex items-center justify-center gap-2'>
        ...
        <span className='px-4 py-2 text-sm text-gray-600 bg-gray-100 rounded-md'>{t('uns.page', { page: unsplashParam.page })}</span>
      ...
      <div className='flex-1 mt-4 overflow-y-auto'>
        ...
        {imageList.length === 0 && (
          <div className='flex flex-col items-center justify-center h-64 text-gray-500'>
            ...
            <p className='text-lg font-medium mb-2'>{t('uns.empty')}</p>
            <p className='text-sm'>{t('uns.emptyHint')}</p>
          </div>
        )}
      </div>
    </div>
